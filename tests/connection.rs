...
fn install_and_remove_module_successful() {
    let connection = SrConnection::new(ConnectionOptions::Datastore_Running).expect("Should be Ok");
    let install =
        connection.install_module(Path::new("./assets/yang/install-test.yang"), None, None);
